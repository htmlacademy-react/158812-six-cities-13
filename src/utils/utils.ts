...
export const calculateRating = (rating: number) => `${Math.round(rating) / COUNT_STARS * 100}%`;
...
export const filterOffersByCity = (offersList: Offer[], city: string | undefined, sorting: string) => offersList.filter((offer) => offer.city.name === city).sort(SortingCallback[sorting]);
